...
import { ProductFilters, CreateProductInput, UpdateProductInput } from '../types/product';
...
export function useProducts(initialFilters?: ProductFilters) {
  ...
  const search = (newFilters: Partial<ProductFilters>) => {
    const updatedFilters = { ...filters, ...newFilters, page: 1 }; // Reset page on new search
  ...
  const updateThumbnail = async (id: string, thumbnail: File | string) => {
    try {
      ...
      return { 
        success: false, 
        error: error.response?.data?.message || error.message || 'Erro ao atualizar imagem' 
      };
    }
  };
